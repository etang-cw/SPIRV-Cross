...
#define PACKED_FORMAT(log2_align, elems, packing, type, order) FORMAT(log2_align, elems, packing, type, true, order, false)
	static constexpr std::pair<MSLFormat, MSLFormatInfo> s_vertex_info_entries[] = {
		...
		{ MSL_FORMAT_A8B8G8R8_SRGB_PACK32,    SIMPLE_FORMAT_VK_PACKED(0, 4, UNorm, RGB) },
		...
		{ MSL_FORMAT_R8_SRGB,          SIMPLE_FORMAT(0, 1, UNorm, RGB) },
		...
		{ MSL_FORMAT_R8G8_SRGB,        SIMPLE_FORMAT(0, 2, UNorm, RGB) },
		...
		{ MSL_FORMAT_R8G8B8_SRGB,      SIMPLE_FORMAT(0, 3, UNorm, RGB) },
		...
		{ MSL_FORMAT_B8G8R8_SRGB,      SIMPLE_FORMAT(0, 3, UNorm, BGR) },
		...
		{ MSL_FORMAT_R8G8B8A8_SRGB,    SIMPLE_FORMAT(0, 4, UNorm, RGB) },
		...
		{ MSL_FORMAT_B8G8R8A8_SRGB,    SIMPLE_FORMAT(0, 4, UNorm, BGR) },
...
#undef FORMAT
	...
	if (static_cast<uint32_t>(format) < s_vertex_info.size()) [[likely]]
                                              ...
                                              const VectorView<MSLVertexBinding> &in_bindings)
{
	...
	for (const MSLVertexAttribute &attr : in_attributes)
	{
		...
		uint8_t align = info.log2_align + (info.vk_packed ? 2 : 0);
...
void CompilerMSL::MSLVertexLoaderWriter::generate(CompilerMSL &out, TypeID main_struct)
{
		...
		std::string msl_name() const
		{
			...
			if (value & PIXEL)
			{
				...
				if (format == RGBA8Unorm || format == RGBA8Snorm || format == RGBA16Unorm || format == RGBA16Snorm)
					out.append("rgba", elems);
				out.append(_msl_name(format));
				out.push_back('<');
				out.append(value & FLOAT ? "float" : "half");
				if (elems > 1)
					out.push_back(static_cast<char>('0' + elems));
				out.push_back('>');
			}
			else
			{
				...
				if (value & FLOAT)
				{
					out.append(_float_name(size));
				}
				else
				{
					if (!(value & SIGNED))
						out.push_back('u');
					out.append(_int_name(size));
				}
				if (elems > 1)
					out.push_back(static_cast<char>('0' + elems));
			}
	...
	struct BindingInfo
	{
		...
		uint8_t base_align;
		uint8_t min_load_align;
		uint8_t max_needed_align;
		bool read_past_stride;
		std::unique_ptr<ByteUsage[], FreeDelete> usage;
	...
	const bool pixel_types_in_storage = out.msl_options.msl_version >= pixel_type_cutoff;
	const bool pixel_type_loads = pixel_types_in_storage && out.msl_options.use_pixel_type_loads;
	...
	for (uint32_t i = 0; i < MaxAttributes; i++)
	{
		...
		Storage storage =
		    Storage::FromMSLFormat(format, pixel_types_in_storage, attribute.load_type->basetype != SPIRType::Half);
		assert(storage.size() == size);
	...
	for (uint32_t i = 0; i < MaxAttributes; i++)
	{
		...
		// 2. as_type cast to adjust type post-load (load type may be different to give compiler better alignment info)
		...
		if (convert_in_type.is_pixel_format())
		{
			...
			if (elems == 3 || (px_fmt == Storage::SRGBA8Unorm && elems < 4))
				load_type = load_type.adjust_element_count(4);
...
void CompilerMSL::emit_custom_functions()
{
	...
	for (const auto &spv_func : spv_function_implementations)
	{
		...
		case SPVFuncImplLoadVertexRG11B10Half:
			...
			begin_scope();
			statement("ushort3 res = ushort3(value << 5, (value >> 6) & 0xffc0, (value >> 16) & 0xff80);");
		...
		case SPVFuncImplLoadVertexRGB9E5Float:
			...
			statement("int3 mantissa = int3(value & 0x1ff, extract_bits(value, 9, 9), extract_bits(value, 18, 9));");
			statement("return float3(mantissa) * exp2(float(exponent));");
			end_scope();
			break;
...
void CompilerMSL::prepare_shader_vertex_loader()
{
	...
	entry_func.add_fixup_hook_in([this, load = std::move(load)]{
		statement(get_name(get<SPIRVariable>(stage_in_var_id).basetype), " ", get_name(stage_in_var_id), " = spvLoadVertex(", load, ");");
